...
for country in BE DE DK GB NL; do
  curl -s "https://www.archlinux.org/mirrorlist/?country=$country&use_mirror_status=on" >> /etc/pacman.d/mirrorlist.local
done;
sed -i 's/^#Server/Server/g' /etc/pacman.d/mirrorlist.local
sed -i '/^#/d' /etc/pacman.d/mirrorlist.local
rankmirrors /etc/pacman.d/mirrorlist.local > /etc/pacman.d/mirrorlist
